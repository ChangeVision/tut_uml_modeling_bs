...
[start=3]
. 1投目のあとの状態遷移を追加する（ <<frame_stm03>> ）。受け取ったピン数（ストライクだったか）によって遷移先は2通りあるので「選択疑似状態」を使って遷移先を分ける。
...
[start=5]
. 2投目のあとの状態遷移を追加する（ <<frame_stm03>> ）。受け取ったピン数（スペアになったか）によって遷移先は2通りあるので「選択疑似状態」を使って遷移先を分ける。
...
振る舞いのモデルに登場する構成要素や要素間の関連を見つけ出し、ゲームの手順を整理しました。
