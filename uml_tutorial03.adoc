...
「終了疑似状態」は、その図における最後の状態（ひとつとは限りません）を示すのに使います。
...
ところが、同じイベントを待ってる遷移先が2つ以上あると、どちらの状態へ背にするのか決定できなくなります（あいまいな状態遷移と呼びます）。
...
アクション「act1」、「act2」、「act3」は、呼び出されたときのイベントやパラメーターが確認できる簡便なメソッドにしておきます（ <<ruby_sample09>> ）。
...
. クラス図を開く。
. 「Sample」クラスクを選択し、プロパティーから「操作」タブを表示する。
